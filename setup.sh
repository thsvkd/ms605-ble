#!/usr/bin/env bash
#
# setup.sh — one-shot environment setup for the MS605 BLE PoC.
#
# Uses uv to create a reproducible virtualenv (Python 3.12, pinned in
# .python-version) and install bleak with the macOS CoreBluetooth backend.
#
set -euo pipefail

cd "$(dirname "$0")"

echo "==> MS605 BLE PoC — setup"

# 1. Ensure uv is available.
if ! command -v uv >/dev/null 2>&1; then
    echo "error: 'uv' is not installed." >&2
    echo "       Install it with:  curl -LsSf https://astral.sh/uv/install.sh | sh" >&2
    echo "       Then re-run:      ./setup.sh" >&2
    exit 1
fi
echo "==> uv $(uv --version | awk '{print $2}') found"

# 2. Remove a stale/foreign virtualenv (the repo shipped a Linux venv that
#    cannot run on macOS). Safe to delete — uv rebuilds .venv from pyproject.
if [ -d "venv" ]; then
    echo "==> removing stale prebuilt 'venv/' (not usable on this machine)"
    rm -rf venv
fi

# 3. Create .venv and install dependencies from pyproject.toml.
echo "==> uv sync (creating .venv + installing bleak)"
uv sync

# 4. Smoke-test the environment offline (no BLE hardware needed).
echo "==> running offline self-tests"
uv run python ms605_ble.py --self-test >/dev/null
uv run python ms605_poc.py --self-test >/dev/null

echo
echo "Setup complete. Run the interactive PoC with:  ./run.sh"
